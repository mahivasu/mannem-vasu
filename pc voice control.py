...
def greet_spidey():
    ...
    if 5 <= hour < 12:
        greeting = "Good Morning Spidey!"
    elif 12 <= hour < 17:
        greeting = "Good Afternoon Spidey!"
    elif 17 <= hour < 21:
        greeting = "Good Evening Spidey!"
    else:
        greeting = "Good Night Spidey!"

    speak(f"{greeting} I am your Samantha")
...
def open_any(name):
    ...
    if name in KNOWN_PLACES:
        path = KNOWN_PLACES[name]
        if os.path.exists(path):
            os.startfile(path)  
            speak(f"Opening {name} folder, Spidey.")
            return
