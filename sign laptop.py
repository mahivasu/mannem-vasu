...
while cap.isOpened():
        ...
        for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
            ...
            if hand_label == "Right":
                if fingers == [0,1,0,0,0]:
                    ...
                    action, voice_text = "Brightness Up", "Brightness Up Spidey"
                    command = lambda: sbc.set_brightness("+10")
                    ...
                    action, voice_text = "Opening Task Manager", "Opening Task Manager Spidey"
                    command = lambda: os.system("taskmgr")

                elif fingers == [0,1,0,0,1]:  
                    action, voice_text = "Mute / Unmute", "Muting Audio Spidey"
                    command = lambda: pyautogui.press("volumemute")

                elif fingers == [0,1,1,0,1]:
                    action, voice_text = "Taking Screenshot", "Screenshot Taken Spidey"
                    command = lambda: pyautogui.screenshot("capture.png")

                elif fingers == [1,0,0,0,1]:
                    action, voice_text = "Next Track", "Next Track Spidey"
                    command = lambda: pyautogui.press("nexttrack")

                elif fingers == [1,1,0,0,1]:
                    action, voice_text = "Previous Track", "Previous Track Spidey"
                    command = lambda: pyautogui.press("prevtrack")

                elif fingers == [0,0,0,0,0]:
                    action, voice_text = "Show Desktop", "Minimizing All Windows Spidey"
                    command = lambda: pyautogui.hotkey("win", "d")
    ...
    if action_display:
        cv2.putText(frame, f"Gesture: {action_display}", (10, 40),
